...
export default function FavoritesPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 pt-6">
      ...
      {loading ? (
        <div className="space-y-4">{[1, 2, 3].map((i) => <div key={i} className="card-tactile h-20 animate-pulse" />)}</div>
      ) : favorites.length === 0 ? (
        <div className="card-tactile p-10 text-center animate-bounce-in">
          ...
          <h3 className="text-xl font-bold text-slate-700 mb-2">{t("favorites.empty")}</h3>
          <p className="text-slate-500 font-semibold mb-4">{t("favorites.emptySub")}</p>
          <Button data-testid="browse-leagues-btn" onClick={() => navigate("/leagues")}
          ...
          {leagueFavs.length > 0 && (
            <section className="mb-8 animate-slide-up">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-xl font-bold text-slate-800">{t("favorites.leagues")}</h2>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {leagueFavs.map((fav) => (
                  <div key={`${fav.type}-${fav.item_id}`} className="card-tactile p-5 flex items-center gap-4">
                    <button data-testid={`fav-league-go-${fav.item_id}`} onClick={() => navigate(`/league/${fav.item_id}`)} className="flex-1 text-left">
                      <div className="font-bold text-slate-800 text-lg">{fav.name}</div>
                    </button>
                    <button data-testid={`remove-fav-${fav.type}-${fav.item_id}`} onClick={() => removeFavorite(fav)}
          ...
          {teamFavs.length > 0 && (
            <section className="animate-slide-up" style={{ animationDelay: "0.05s" }}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-xl font-bold text-slate-800">{t("favorites.teams")}</h2>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {teamFavs.map((fav) => (
                  <div key={`${fav.type}-${fav.item_id}`} className="card-tactile p-5 flex items-center gap-4">
                    {fav.crest && <img src={fav.crest} alt="" className="w-10 h-10 object-contain flex-shrink-0" />}
                    <button data-testid={`fav-team-go-${fav.item_id}`}
                      onClick={() => fav.league_code ? navigate(`/league/${fav.league_code}`) : null} className="flex-1 text-left">
