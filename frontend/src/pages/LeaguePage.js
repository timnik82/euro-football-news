...
import { ArrowLeft, Heart, Star, Trophy, ChevronRight } from "lucide-react";
...
export default function LeagueDetail() {
  ...
  const toggleTeamFavorite = async (team) => {
    if (!user) { toast.info("Log in to save favorites!"); navigate("/login"); return; }
    try {
      const { data } = await axios.post(
        `${API}/favorites`,
      ...
      if (data.action === "added") {
        ...
        toast.success(`${team.shortName || team.name} added to favorites!`);
      } else {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 pt-6">
      ...
      <div className="flex items-center gap-3 mb-6 animate-slide-up">
        <button
          data-testid="league-back-button"
          onClick={() => navigate(-1)}
        ...
        <div className="flex-1">
          <h1 className="text-3xl sm:text-4xl font-black tracking-tight" style={{ color }}>
            {leagueName}
          </h1>
        </div>
        <button
          data-testid="league-favorite-toggle"
        ...
        <Tabs defaultValue="standings" className="animate-slide-up" style={{ animationDelay: "0.05s" }}>
          ...
          <TabsContent value="standings">
            {allStandingsTable.length === 0 ? (
              <div className="card-tactile p-8 text-center">
                <p className="text-slate-500 font-semibold">Standings not available yet</p>
              </div>
            ) : (
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b-2 border-slate-200">
                        ...
                        <th className="text-left p-3 font-bold text-slate-500">Team</th>
                        <th className="text-center p-3 font-bold text-slate-500 w-10">P</th>
                        <th className="text-center p-3 font-bold text-slate-500 w-10">W</th>
                        <th className="text-center p-3 font-bold text-slate-500 w-10">D</th>
                        <th className="text-center p-3 font-bold text-slate-500 w-10">L</th>
                        <th className="text-center p-3 font-bold text-slate-500 w-12">GD</th>
                        <th className="text-center p-3 font-bold text-slate-800 w-12">Pts</th>
                    ...
                    <tbody>
                      {allStandingsTable.map((row, i) => (
                        ...
                        >
                          ...
                          <td className="text-center p-3 font-black text-lg" style={{ color }}>
                            {row.points}
                          </td>
                          <td className="p-3">
                            <button
                              data-testid={`fav-team-${row.team?.id}`}
                              onClick={() => toggleTeamFavorite(row.team)}
          ...
          <TabsContent value="matches">
            {upcomingMatches.length > 0 && (
              <div className="mb-6">
                <h3 className="text-xl font-bold text-slate-800 mb-3 flex items-center gap-2">
                  <Star size={20} strokeWidth={2.5} className="text-sky-500" /> Upcoming
                </h3>
            ...
            {recentMatches.length > 0 && (
              <div>
                <h3 className="text-xl font-bold text-slate-800 mb-3">Recent Results</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {upcomingMatches.length === 0 && recentMatches.length === 0 && (
              <div className="card-tactile p-8 text-center">
                <p className="text-slate-500 font-semibold">No matches available</p>
              </div>
            )}
          </TabsContent>
          ...
          <TabsContent value="scorers">
            {scorers.length === 0 ? (
              <div className="card-tactile p-8 text-center">
                <p className="text-slate-500 font-semibold">Top scorers not available for this competition</p>
              </div>
            ) : (
              <div className="card-tactile overflow-hidden">
                <div className="p-4 border-b-2 border-slate-200 flex items-center gap-3">
                  <img
                    src="https://images.unsplash.com/photo-1518091043644-c1d4457512c6?crop=entropy&cs=srgb&fm=jpg&ixid=M3w4NjAzNTl8MHwxfHNlYXJjaHwxfHxmb290YmFsbCUyMHRyb3BoeXxlbnwwfHx8fDE3NzU1OTY2MTd8MA&ixlib=rb-4.1.0&q=85"
                    alt="Trophy"
                    className="w-10 h-10 rounded-xl object-cover"
                  />
                  <h3 className="text-lg font-black text-slate-800">Top Scorers</h3>
                </div>
                <div>
                  {scorers.map((scorer, i) => (
                    ...
                    >
                      ...
                      <div className="text-right flex-shrink-0">
                        ...
                        <div className="text-xs font-bold text-slate-400">{scorer.assists || 0} ast</div>
                      </div>
                    </div>
                  ))}
                </div>
